...
-- studiousb_protocol.fields.u32 = ProtoField.uint32("studiousb.u32", "generic u32", base.HEX)
...
function studiousb_protocol.dissector(buf, pinfo, tree)
	...
	local selector = buf(0,4):uint() -- field 'a' in kernel code
	local field_b = buf(4,4):uint()
	local field_f1 = buf(8,4):uint()
	local field_f2 = buf(12,4):uint()
	...
	subtree:add(u32, buf(8,4)):set_text(string.format('F1 marker: %X', field_f1))
